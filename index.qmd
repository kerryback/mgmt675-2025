...
In some cases, we will use generative AI (LLMs) only to write the python code.  We could, in such cases, export the python code to run on our own computers or elsewhere in the cloud and not need an LLM for subsequent use of the code.  We will explore how to do that in the course, though it will not be the focus of the course.  In other cases, generative AI may be an integral part of the workflow.  For example, we may use AI to summarize the MD&A section of a 10-K.  In those cases as well, we could export the python code and run it elsewhere, but we would need to add code that accesses the LLM's API to make requests (and we would need a paid account at the LLM provider to do that).  We will illustrate that in the course also, using Julius to write the code to send requests and get responses from the LLM.  If we wanted to take an additional step and get into software development, which of course we don't in this course, then we could create a Windows or Mac executable that will open a browser window, asking the user for input (e.g., a ticker or an API key if we are sending requests to an LLM), and then running the workflow and returning results, without the user needing to run or even know anything about python. Again, Julius can write all of the code to create that.
