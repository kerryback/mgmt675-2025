...
A great feature of working with Julius is that it is interactive.  After running a workflow, you may think of some additional analysis you want to perform.  You can make the request to Julius in plain English and get the additional analysis you want.

In some cases, we may not need to work interactively.  For example, I built a workflow to scrape the SEC EDGAR site that only requires the user to provide a ticker ([SEC EDGAR 10-Q Scraper](https://julius.ai/workflows/preview/02bcd09a-81be-4531-9cd5-58fc5296630d)).  In such cases, we could export the python code to run on our own computers.  If generative AI is an integral part of the workflow (e.g., if we are using AI to summarize the MD&A section of a 10-K), then  we would need to add code that accesses the LLM's API to make requests (and we would need a paid account at the LLM provider to do that).  We can also use Julius to write the code to do that.  If we wanted to take an additional step and get into software development, which of course we don't in this course, then we could create a Windows or Mac executable that will open a browser window, asking the user for input (e.g., a ticker or an API key if we are sending requests to an LLM), and then running the workflow and returning results, without the user needing to run or even know anything about python. Again, Julius can write all of the code to create that.
